...
export class SyncCommitter {
  ...
  async commitSubmission(
    ...
    return this.commitFiles(
      files,
      message,
      new Date(submissionTimestamp * 1000)
    );
  }

  /**
   * Creates the optional post-sync commit (e.g. an index generated by a
   * `post` hook) chained after the last submission commit.
   */
  async commitPostSync(files: CommitFile[], message: string): Promise<void> {
    return this.commitFiles(files, message, new Date());
  }
